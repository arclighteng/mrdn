...
// GetScoreHeatmap returns the average latest composite score and company count
// per sector. Companies with no scores are excluded. Sectors are ordered by
// average composite score descending.
func (s *Store) GetScoreHeatmap(ctx context.Context) ([]HeatmapEntry, error) {
	rows, err := s.db.Query(ctx, `
		WITH latest_scores AS (
			SELECT DISTINCT ON (company_id)
				company_id, composite_score
			FROM scores
	...
	for rows.Next() {
		var e HeatmapEntry
		if err := rows.Scan(&e.Sector, &e.AvgComposite, &e.CompanyCount); err != nil {
